    ...
    ordererInfo.insertAdjacentHTML("beforeend", `
      <div>
        ...
        <div class="form-group">
          ...
          <input type="email" class="form-control" id="email" placeholder="이메일" value=${email} disabled />
        </div>
      </div>`);
  })
  ...
  method: "POST",
    ...
    .then((data) => {
      ...
      if(data.status === 201) {
        ...
        window.location.href = `../orderComplete/orderComplete.html?orderId=${data.orderId}`;
      }
    })
    .catch((error) => console.error(error));
});
