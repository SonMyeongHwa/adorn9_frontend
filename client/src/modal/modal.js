...
if (cartItems.length === 0) {
	...
	cartItems.forEach((cartItem) => {
      ...
      <tr>
        ...
        <td class="td-top">
          <button id='remove-btn-${id}' class="delete-btn" onclick="removeItem(this)">X</button>
        </td>
      </tr> `,
		);
	});
	...
	if (checkedCount > 0) {
		if (confirm('선택하신 상품을 삭제하시겠습니까?')) {
			...
			checkedItems.forEach((checkedItem) => {
				...
				basket = JSON.parse(localStorage.getItem('item')).filter(
					(param) => param.id !== _id,
				);
				localStorage.clear();
				localStorage.setItem('item', JSON.stringify(basket));
